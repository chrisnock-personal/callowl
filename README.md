...
- **Tested backup/restore drill + point-in-time recovery** *(production-readiness)* — scheduled `pg_dump`s exist and restore works (verified this session), but there's no rehearsed disaster-recovery runbook and no point-in-time recovery (WAL archiving) — a `pg_dump` only ever restores to the moment of the last dump, not to an arbitrary point since then.
