...
**Actually recovering from an incident** — see **[DISASTER_RECOVERY.md](./DISASTER_RECOVERY.md)** for step-by-step procedures (restoring onto a still-running stack vs. rebuilding a fully lost host), a post-restore verification checklist, and this setup's known limitations (no point-in-time recovery yet, backups aren't copied off-host automatically). Rehearsed against a scratch database, not just written from memory.
...
- **Multi-replica-safe scheduled jobs** *(production-readiness)* — the audit-log pruner, `remote_source_rejects` pruner, and remote-source poller (`pollDueRemoteSources`, `backend/src/index.ts`) are all in-process `setInterval`s tied to a single backend instance. Scaling to 2+ backend replicas today would fire all three redundantly — duplicate remote polls (harmless since ingest is idempotent by `callId`, but wasteful, and could double-run a custom script's real side effects), duplicate prune queries (harmless, just redundant `DELETE`s). Needs a real story before this can run as more than one replica: leader election, a dedicated scheduler process, or a job queue with locking.
- **Native TLS termination, or an explicit "always needs a reverse proxy" decision** *(production-readiness)* — currently always bring-your-own (see [Deploying](#deploying)). Worth deciding explicitly whether this should ever terminate TLS itself (e.g. a bundled Caddy sidecar with auto-provisioned certs) for a true single-command production deploy, or whether "always needs an external reverse proxy" is the permanent, accepted design — right now it reads more like an omission than a decision.
- **Secrets rotation** *(production-readiness)* — none of `ADMIN_API_KEY`/`INGEST_API_KEY`/`REMOTE_SOURCE_ENC_KEY` have a rotation story; rotating `REMOTE_SOURCE_ENC_KEY` specifically invalidates every already-stored remote-source credential (an accepted v1 limitation when that feature shipped). Needs at minimum a documented rotation runbook, ideally dual-key support during a rotation window.
- **Observability: metrics + structured logging** *(production-readiness)* — currently just `console.log`/`console.error` to stdout and a bare `/health` status/version probe. No metrics export (request latency, error rates, poll success/failure rates), no log aggregation, no error tracking — needed to actually run this unattended rather than by someone tailing `podman logs`.
- **Login hardening: account lockout + MFA** *(production-readiness)* — login is rate-limited (20 attempts/15min/IP, see [Deploying](#deploying)) but not locked out per-account, and there's no MFA option. Rate limiting alone doesn't stop a slow, patient distributed attempt.
- **Point-in-time recovery (WAL archiving)** *(production-readiness)* — a `pg_dump` only ever restores to the exact moment it was taken; anything written since is gone. Continuous WAL archiving (e.g. `pgBackRest`/`WAL-G`, or Postgres's own `archive_command`) would close that gap, but needs a real infra addition (an archive target — S3-compatible storage or similar — plus a restore procedure meaningfully more involved than `pg_restore`) — split off from the backup/restore drill (see Done below) as separate, deferred work.

Done:

- ~~Tested backup/restore drill + runbook~~ — **[DISASTER_RECOVERY.md](./DISASTER_RECOVERY.md)** documents restore-onto-a-running-stack vs. full-host-loss procedures, a post-restore verification checklist, and known limitations (no PITR yet — see the new bullet above; backups aren't copied off-host automatically; no partial restore). Actually rehearsed, not just written from memory: a real backup of the live database was restored into a scratch database and the checklist run against it, confirming an exact match (row counts, migrations table, a byte-for-byte record spot-check, intact user/password-hash) while the real database was untouched throughout. Also fixed a real gap the drill surfaced: the scheduled `backup` service's failures used to only ever reach `podman logs` — `scripts/backup.sh` now writes a `.last-attempt` status marker (success or failure) that `GET /admin/backups` surfaces, and the dashboard's Backups panel now shows a warning when the last backup is overdue or its last scheduled attempt failed, instead of rendering identically to a healthy state.
