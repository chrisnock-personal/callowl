...
- **Secrets rotation** *(production-readiness)* — none of `ADMIN_API_KEY`/`INGEST_API_KEY`/`REMOTE_SOURCE_ENC_KEY`/`MFA_ENC_KEY` have a rotation story; rotating either encryption key specifically invalidates every already-stored remote-source credential or MFA secret respectively (an accepted v1 limitation when each feature shipped — deliberately separate keys precisely so rotating one doesn't force the other, see the MFA entry below). Needs at minimum a documented rotation runbook, ideally dual-key support during a rotation window.
